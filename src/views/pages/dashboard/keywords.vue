              ...
              <form>
                <div class="form-row">
                  <div class="form-group col-md-12">
                    <label for="inputName">{{
                      $t("dashboard.keywords.name")
                    }}</label
                    >&nbsp;
                    <p-popover-labels :text="$t('dashboard.keywords.nameD')" />
                    <button
                      type="button"
                      class="btn btn-outline-dark btn-sm add styleBtn"
                      @click="back()"
                      style=""
                    >
                  ...
                  <div class="form-group col-md-12">
                    <a class="btn btn-dark styleBtn" @click="Upload()">{{
                      $t("dashboard.keywords.submit")
                    }}</a>
                  </div>
                </div>
              </form>
          ...
          <div class="card-body">
            ...
            <div class="card-text">
              <div class="row" v-for="k in keywords">
                ...
                <div
                  class="col-sm-5"
                  v-if="user.is_the_admin"
                  style="color: #787878"
                >
                  {{ nameUsers[k.user_id_creator] }}
                </div>
...
export default {
  ...
  methods: {
    ...
    deleteKeyword(keyword) {
      const vm = this;
      Api()
        .delete("/api/keyword/" + keyword.keyword_id)
        .then(
          function (response) {
            vm.updateKeyword();
          ...
          function (cause) {
            ...
            if (cause.response.status === 403)
              msg = "The administrator is who can update/delete the keyword.";
    ...
    updateKeyword() {
      ...
      Api()
        ...
        .then(function (all) {
          Api()
            .get("/api/keyword/?user_id_creator=" + vm.user.user_id)
