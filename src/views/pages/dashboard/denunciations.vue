              ...
              >
                ...
                <div class="col-sm-2">
                  <button
                    type="button"
                    class="btn btn-outline-danger btn-sm add"
                    @click="deleteNotification(comment)"
                  >
                    <md-icon>clear</md-icon>
                  </button>
                </div>
      ...
      <div class="col-sm-8">
        <div class="card text-center" v-if="is_comment">
          <div class="card-header">
            ...
            <ul class="description">
              <li>
                <b> User Name:</b>
                {{ nameUsers[chosenComment.user_id_creator] }}
              </li>
              <li>
                <b>{{ $t("map.viewInfo.lbDescription") }}:</b
                >{{ chosenComment.description }}
              </li>
              <li>
                <b>{{ $t("map.viewInfo.lbDate") }}:</b>
                {{ chosenComment.created_at }}
              </li>
        ...
        <div class="card text-center" v-if="is_layer">
          <div class="card-header">
            ...
            <ul class="description">
              <li>
                <b> {{ $t("map.viewInfo.lbTitle") }}:</b> {{ chosenLayer.name }}
              </li>
              <li>
                <b> {{ $t("map.viewInfo.lbKeywods") }}:</b>
                <el-tag
                  v-show="is_layer"
                  v-for="id in chosenLayer.keyword"
                  :key="'tag' + id"
              ...
              <li>
                <b>{{ $t("map.viewInfo.lbDescription") }}:</b>
                {{ chosenLayer.description }}
              </li>
              <li>
                <b>{{ $t("map.viewInfo.lbDate") }}:</b>
                {{ chosenLayer.created_at }}
              </li>
              <li v-if="chosenLayer.reference === []">
                <b>{{ $t("map.viewInfo.lbReferences") }}:</b>
                <span
                  v-show="is_layer"
                  v-for="id in chosenLayer.reference"
                  :key="'ref' + id"
                >
                  {{ allReferences[id] }};
                </span>
  ...
  methods: {
    ...
    selectLayer(id) {
      ...
      vm.denunciations = Object.values(this.notifications).filter((notif) => {
        return (
          notif.layer_id === id &&
          notif.notification_id_parent === null &&
          notif.is_denunciation === true
        );
      });
    },
    selectComment(id) {
      ...
      this.denunciations = Object.values(this.notifications).filter((notif) => {
        return (
          notif.notification_id_parent === id && notif.is_denunciation === true
        );
      });
      vm.nameLayer =
        vm.chosenComment.layer_id === null
          ? "Global"
          : vm.layers[vm.chosenComment.layer_id].name;
    },
    clean() {
      const vm = this;
      this.chosenComment = [];
      ...
      Api()
        ...
        .then(function (keys) {
          keys.data.features.forEach((key) => {
            vm.allKeywords = {
              ...vm.allKeywords,
              [key.properties.keyword_id]: key.properties.name,
            };
          });
        });
      Api()
        ...
        .then(function (references) {
          references.data.features.forEach((reference) => {
            vm.allReferences = {
              ...vm.allReferences,
              [reference.properties.reference_id]:
    ...
    deleteNotification(notification) {
      ...
      Api()
        .delete("/api/notification/?notification_id=" + notification.id)
        .then(function (response) {
          ...
          Api()
            ...
            .then(function (layers) {
              layers.data.features.forEach((layer) => {
                vm.nameLayers = {
                  ...vm.nameLayers,
                  [layer.properties.layer_id]: layer.properties.name,
                };
                vm.layers = {
                  ...vm.layers,
                  [layer.properties.layer_id]: layer.properties,
                };
              });
            });
          setTimeout((_) => {
            vm.denouncedLayers = response.data.features.reduce(
              (acc, currvalue) => {
                ...
                return {
                  ...acc,
                  [id]: { n: idCont, name: vm.nameLayers[id], id: id },
                };
              },
              {},
            );
          ...
          Api()
            ...
            .then(function (users) {
              users.data.features.forEach((user) => {
                vm.nameUsers = {
                  ...vm.nameUsers,
                  [user.properties.user_id]: user.properties.name,
                };
              });
            });
          setTimeout((_) => {
            vm.denouncedComments = response.data.features
              .filter((e) => {
                return (
                  e.properties.notification_id_parent !== null &&
                  e.properties.is_denunciation === true
                );
              })
              .reduce((acc, currvalue) => {
                ...
                return {
                  ...
                  [id]: {
                    ...
                    name: vm.nameUsers[
                      vm.notifications[
                        currvalue.properties.notification_id_parent
                      ].user_id_creator
                    ],
                    id: id,
                  },
