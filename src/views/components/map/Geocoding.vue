  ...
  <section class="box" v-show="boxGeocoding">
    ...
    <div class="box-multigeocoding" v-show="multigeocoding">
      ...
      <label class="file-select">
        ...
        <input type="file" accept=".csv" @change="handleFileChange" /> </label
      ><br /><br />

      <form
        class="headers-form"
        v-show="headers.length > 0"
        @submit.prevent="visualizar()"
      >
        <div class="inputs">
...
export default {
  ...
  async mounted() {
    try {
      ...
      this.$alert(
        "Serviço de GEOCODING indisponível, tente mais tarde ou comunique nosso suporte!",
        "Erro Interno",
        {
  ...
  methods: {
    ...
    querySearch(queryString, cb) {
      ...
      let results = queryString
        ? links.filter(
            (link) =>
              link.toLowerCase().indexOf(queryString.toLowerCase()) >= 0,
          )
        : links;
      cb(results);
    },
    ...
    handleFileChange(event) {
      ...
      reader.onload = async (_file) => {
        ...
        if (headers.some((e) => e === "")) {
          this._msgError(
            "Há nome de coluna(s) (i.e. cabeçalho(s)) em branco. Dê um nome a ela(s) ou remova-a(s)!",
          );
          return;
        }
    ...
    async visualizar() {      
      ...
      for (let i = 0; i < json.length; i++) {
        let address =
          json[i][this.street].toLowerCase() +
          ", " +
      ...
      try {
        ...
        if (regex.test(search)) {
          ...
          if (result.data[1][0].geom != undefined) {
            ...
            let coordPoint = result.data[1][0].geom
              .substring(6)
              .replace(")", "")
              .split(" ");
          ...
          this.$alert(
            "<strong>Pesquise por:</strong> rua, número, ano (0000)",
            "Formato inválido",
            {
              dangerouslyUseHTMLString: true,
              confirmButtonText: "OK",
        ...
        if (
          ...
          error.response.data[1][0].alertMsg != undefined
        ) {
