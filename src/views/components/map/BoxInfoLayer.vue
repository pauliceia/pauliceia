  ...
  <div class="box-info" v-show="boxInfoLayer">
    ...
    <div class="body">
      <ul class="description">
        <li>
          <b> {{ $t("map.viewInfo.lbKeywods") }}:</b>
          <el-tag
            v-show="layer != null"
            v-for="id in infos.keywords"
            :key="'tag' + id"
        ...
        <li>
          <b>{{ $t("map.viewInfo.lbDescription") }}:</b> {{ infos.description }}
        </li>
        <li>
          <b>{{ $t("map.viewInfo.lbDate") }}:</b> {{ infos.date }}
        </li>
        <li>
          <b>{{ $t("map.viewInfo.lbAuthors") }}:</b>
          <span
            v-show="layer != null"
            v-for="author in infos.authors"
            :key="author.properties.user_id"
          >
      ...
      <div class="nofitication">
        <md-list-item>
          ...
          <p class="md-list-item-text">
            {{ $t("map.viewInfo.lbNotifications") }}:
          </p>
        </md-list-item>
        <div class="notification-box">
          ...
          <div style="right: 60px; position: absolute">
            <a
              ...
              @click="addNotif()"
              style="color: white"
              >{{ $t("map.viewInfo.submit") }}</a
            >
          </div>
          <div style="right: 150px; position: absolute">
            <a class="btn btn-warning" @click="addNotifD()"
              ><md-icon>report</md-icon></a
            >
          </div>
          ...
          <div class="notification-box">
            ...
            <p class="comments">
              ...
              <button
                ...
                class="btn btn-outline-danger btn-sm add"
                @click="clearNot(n)"
                v-show="user.user_id === n.user_id_creator || user.is_the_admin"
              >
                <md-icon>clear</md-icon>
              </button>
            </p>
            <div class="msgType">
              ...
              <div v-else-if="n.notification_id_parent !== null">
                Reply&nbsp;
              </div>
              <div
                v-if="
                  n.nameParent === null &&
                  (n.is_denunciation !== false ||
                    n.notification_id_parent !== null)
                "
              >
                of Layer
              </div>
              <div v-if="n.nameParent !== null">
                of {{ n.nameParent }}'s message
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    updateNotif() {
      ...
      Api()
        .get("/api/notification/?layer_id=" + vm.id)
              ...
              .then(function (userInfo) {
                if (notification.properties.notification_id_parent === null) {
                  ...
                  Api()
                    ...
                    .then(function (notificationParent) {
                      Api()
                        .get(
                          "/api/user/?user_id=" +
                            notificationParent.data.features[0].properties
                              .user_id_creator,
                        )
                        .then(function (userInfoParent) {
                          vm.notifG.push({
                            description: notification.properties.description,
                            name: userInfo.data.features[0].properties.name,
                            date: notification.properties.created_at,
                            ...
                            notification_id_parent:
                              notification.properties.notification_id_parent,
                            nameParent:
                              userInfoParent.data.features[0].properties.name,
                          });
                          setTimeout((_) => {
    ...
    followLayer() {
      ...
      Api()
        .post("/api/layer_follower/create", layer_follower)
        .then(
          function (response) {
          ...
          function (cause) {
            if (cause.response.status === 409)
              msg =
                "The user can't follow the layer, because you are a collaborator or owner of the layer.";
      ...
      Api()
        ...
        .then(
          ...
          function (cause) {
            if (cause.response.status === 409)
              msg =
                "The user can't follow a layer, because he/she is a collaborator or owner of it.";
            else if (cause.response.status === 406)
              msg =
                "The user can't follow a layer, because he/she already follow it.";
            else msg = cause.toString();
            vm.$message.error(msg);
          },
        );
    ...
    getReferenceDescription(id) {
      return this.allReferences.filter(
        (reference) => reference.properties.reference_id === id,
      );
    },
    getAuthorName(item) {
      return this.allAuthors.filter(
        (author) => author.properties.user_id === item.properties.user_id,
      );
    },
    async _getInfos() {
      ...
      this.infos.authors = this.allAuthorsLayers.filter(
        (item) => item.properties.layer_id == this.layer.layer_id,
      );
