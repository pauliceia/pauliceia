...
function extractYearFromDateAsString(date, default_value) {
  ...
  if (splittedDate.length === 3) {
    ...
    if (splittedDate[0].length === 4) year = parseInt(splittedDate[0]);
    else if (splittedDate[1].length === 4) year = parseInt(splittedDate[1]);
  }
...
export default {
  ...
  methods: {
    getTemporalColumns(f_table_name) {
      ...
      let tcs = this.loadedTC.filter(
        (tc) => tc.properties.f_table_name === f_table_name,
      );

      // if array is not empty, it returns the only one element inside the array
      if (Array.isArray(tcs) && tcs.length) return tcs[0];
      else return null;
    },
    updateFeatureVisibility(vectorLayerFeatures, tc) {
      vectorLayerFeatures.forEach((feature) => {
        ...
        let startYear = extractYearFromDateAsString(
          startDate,
          this.sliderStartYear,
        );
        ...
        if (
          startYear <= this.selectedEndYear &&
          endYear >= this.selectedStartYear
        ) {
    ...
    filterUpdate() {
      overlayGroup.getLayers().forEach((vectorLayer) => {
        ...
        if (tc) {
          ...
          Api()
            .get("/api/temporal_columns/?f_table_name=" + f_table_name)
            .then((result) => {
