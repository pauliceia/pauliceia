<template>
  <div :class="boxView ? 'box-item active' : 'box-item'">
    ...
    <div v-show="boxView && layerStatus" class="box-options">
      ...
      >
        <md-icon
          :class="getInfo ? 'active' : ''"
          @click.native="setStatusGetInfo()"
          >info</md-icon
        >
      </el-tooltip>
...
export default {
  ...
  watch: {
    ...
    //edit color of layer
    colorVector(val) {
      if (val == null) this.colorVector = "rgba(255,255,255,0)";
      else
        this.group.getLayers().forEach((sublayer) => {
    ...
    async getColor() {
      ...
      for (let layer of this.group.getLayers().getArray()) {
        if (layer.get("title") === this.title) {
          ...
          if (this.type === "multilinestring" || this.type === "linestring") {
            if (styleType === "function") layer.setStyle(lineStyle);

            this.colorVector = lineStyle.getStroke().getColor();
          } else if (this.type === "multipoint" || this.type === "point") {
            if (styleType === "function") layer.setStyle(pointStyle);

            this.colorVector = pointStyle.getImage().getFill().getColor();
          } else if (this.type === "multipolygon" || this.type === "polygon") {
            if (styleType === "function") layer.setStyle(polygonStyle);
    ...
    extend() {
      this.group.getLayers().forEach((sublayer) => {
        if (sublayer.get("title") === this.title && this.layerStatus) {
          ...
          this.$root.olmap
            .getView()
            .fit(extentEmpty, this.$root.olmap.getSize());
        }
      });
    },
    infosLayer() {
    ...
    _getInfosFeatures() {
      ...
      vm.select.on("select", (event) => {
        ...
        event.selected
          .filter(
            (feature) =>
              feature.getId().split(".")[0] == vm.title.toLowerCase(),
          )
          .forEach((feat) => {
            feat.setStyle();
