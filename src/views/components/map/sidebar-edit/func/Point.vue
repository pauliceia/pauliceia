<template>
  <div class="box-func-edit">
    <el-radio-group v-model="option" size="medium">
      <el-radio-button label="ADD" @click.native="add()"
        ><i class="el-icon-plus"></i> ADD</el-radio-button
      >
      <el-radio-button label="EDIT" @click.native="edit()"
        ><i class="el-icon-edit"></i> EDIT</el-radio-button
      >
      <el-radio-button label="DEL" @click.native="remove()"
        ><i class="el-icon-close"></i> DEL</el-radio-button
      >
    </el-radio-group>
  </div>
</template>
    ...
    edit() {
      ...
      this.selectEdit.on("select", (event) => {
        ...
        if (featureSelect.length != 0)
          if (
            featureSelect[0].getId() == undefined ||
            featureSelect[0].getId() == "waiting"
          )
      ...
      this.selectRemove.on("select", (event) => {
        ...
        if (featureSelect.length != 0) {
          if (
            ...
            featureSelect[0].setStyle();
            this.$confirm(
              "Você irá remover a feature selecionada. Deseja continuar?",
              "Warning",
              {
              ...
              .then(async (_) => {
                try {
                  let layerName = featureSelect[0]
                    .getId()
                    .substr(0, featureSelect[0].getId().lastIndexOf("."));
                  let featureId = featureSelect[0]
                    .getId()
                    .substr(featureSelect[0].getId().lastIndexOf(".") + 1);
                  let response = await Edit.deleteFeature(
                    layerName,
                    featureId,
                    vm.changesetId,
                  );
                  ...
                  vm.$message({
                    message:
                      "Erro na plataforma, não foi possível excluir o vetor!",
                    type: "error",
                  });
                }
              })
