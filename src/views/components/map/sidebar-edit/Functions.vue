<template>
  <section class="box-function" v-show="layerId != null">
    ...
    <center>
      <p-sidebarEdit-func-point
        v-show="layer.type == 'point' || layer.type == 'multipoint'"
        :source="layer.vector"
      />
      <p-sidebarEdit-func-line
        v-show="layer.type == 'multilinestring' || layer.type == 'linestring'"
        :source="layer.vector"
      />
      <p-sidebarEdit-func-polygon
        v-show="layer.type == 'multipolygon' || layer.type == 'polygon'"
      />
...
export default {
  ...
  methods: {
    async _getLayer(id) {
      ...
      let infoFeatureTable = await Dashboard.getFeatureTable(
      ...
      overlayGroup.getLayers().forEach((sublayer) => {
        if (sublayer.get("title") === properties.f_table_name) {
          this.layer.type =
            infoFeatureTable.data.features[0].geometry.type.toLowerCase();
