<template>
  <section class="box-edit">
    <div class="box-layers">
      <div
        v-if="
          user != null && user.user_id !== undefined && user.user_id != null
        "
      >
        <p class="title">
        ...
        <el-radio-group v-model="layerIdSelected">
          <div class="layers" v-for="layerId in myLayersId" :key="layerId">
            <p-sidebarEdit-item
              :id="layerId"
              @finishEdit="(id) => closeChangeset(id)"
            />
          </div>
        </el-radio-group>
  ...
  async mounted() {
    if (
      ...
      if (response.status == 200)
        this.myLayersId = response.data.features.map(
          (layer) => layer.properties.layer_id,
        );
    }
  },
  methods: {
    _alterLayer(layerId) {
      if (this.layerIdSelected != null)
        if (this.lastLayerIdSelected == null) {
          ...
          this.$alert(
            "Você está editando outra layer, favor finalize a primeira antes!",
            "Erro",
            {
    ...
    _createChangeset(layerId) {
      ...
      this.$msgbox({
        ...
        this.$message({
          type: "warning",
          message:
            'EDIÇÃO inicializada com sucesso, ao terminar não se esqueça de clicar no botão "FINALIZAR", para concluir!',
        });
      });
    },
