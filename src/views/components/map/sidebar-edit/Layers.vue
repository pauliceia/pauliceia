    ...
    <div class="box-layers">
      <div v-if="user != null && user.user_id !== undefined && user.user_id != null">
        ...
        <div v-if="myLayersId.length == 0">
          <el-alert type="info" center :closable="false" show-icon
            title="Você não possui camadas associadas ao seu usuário, adicione para poder editar!" />
        </div>
      </div>
      <div v-else>
        <el-alert title="ERRO! Você não tem permissão para acessar esse recurso." type="error" center show-icon></el-alert>
      </div>
    </div>
  </section>
</template>
