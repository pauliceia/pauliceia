<template>
  <div>
    <el-button type="text" @click="showModal2()">{{
      $t("dashboard.notification.comments")
    }}</el-button>

    <el-dialog
      :title="$t('dashboard.notification.notification')"
    ...
    >
      ...
      <div class="notification-box2" v-if="false">
        ...
        <div style="right: 30px; position: absolute">
          <a class="btn btn-primary" @click="addNotif()" style="color: white">{{
            $t("dashboard.notification.comments")
          }}</a>
        </div>
          ...
          <div class="notification-box2">
            ...
            <p class="comments">
              ...
              <button
                ...
                class="btn btn-outline-danger btn-sm add"
                @click="clearNot(n)"
                v-if="user.user_id === n.user_id_creator || user.is_the_admin"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="outerVisible = false">{{
          $t("dashboard.notification.close")
        }}</el-button>
...
export default {
  ...
  methods: {
    ...
    updateNotif() {
      ...
      Api()
        .get("/api/notification/?notification_id=" + vm.notification_id)
        .then(function (notification) {
          Api()
            .get(
              "/api/user/?user_id=" +
                notification.data.features[0].properties.user_id_creator,
            )
            .then(function (user) {
              vm.notification = {
                description:
                  notification.data.features[0].properties.description,
                name: user.data.features[0].properties.name,
                created_at: notification.data.features[0].properties.created_at,
                type: "myLayer",
                notification_id:
                  notification.data.features[0].properties.notification_id,
                user_id_creator:
                  notification.data.features[0].properties.user_id_creator,
                is_denunciation:
                  notification.data.features[0].properties.is_denunciation,
                ...
                notification_id_parent:
                  notification.data.features[0].properties
                    .notification_id_parent,
              };
      ...
      Api()
        .get("/api/notification/?notification_id_parent=" + vm.notification_id)
